...
unsigned int Startup_Ramp = 0; 		/* Start up ramp in open loop. This variable
...
unsigned short Startup_Lock = 0;	/* This is a counter that is incremented in
...
unsigned short iADCisrCnt = 0;    // This Counter is used as a timeout for polling the push buttons
...
unsigned short AccumThetaCnt = 0; // Counter used to calculate motor speed. Is incremented
...
float qVdSquared = 0; 			  // This variable is used to know what is left from the VqVd vector
...
float DCbus = 0;      			  // DC Bus measured continuously and stored in this variable
...
float TargetDCbus = 0;			  // DC Bus is measured before running motor and stored in this
                        		  // variable. Any variation on DC bus will be compared to this value
                        		  // and compensated linearly.

float Theta_error = 0;		  	  // This value is used to transition from open loop to closed looop.
...
float ADCBuffer[4];     // Buffer used to store ADC Samples
