...

{
    ...
    while(length)                         /* read till length is 0 */
    {
        ...
        wait=0;
        if(U4MODEbits.PDSEL == 3)         /* check if TX/RX is 8bits or 9bits */
            *buffer++ = U4RXREG;          /* data word from HW buffer to SW buffer */
	else
            *temp_ptr++ = U4RXREG & 0xFF; /* data byte from HW buffer to SW buffer */
